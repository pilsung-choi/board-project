...
describe('MovieService - Integration Test', () => {
  ...
  describe('findAll', () => {
    it('should return movies with correct titles', async () => {
      ...
      const result = await service.getManyMovies(dto);
    ...
    it('should return likeStatus if userId is provided', async () => {
      ...
      const result = await service.getManyMovies(dto, users[0].id);
  ...
  describe('findOne', () => {
    ...
    it('should throw NotFoundException if movie does not exist', async () => {
      await expect(service.getMovieById(999999999999)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
